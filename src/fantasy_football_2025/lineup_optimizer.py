...
class LineupOptimizer:
    ...
    def load_player_data(self) -> pd.DataFrame:
        ...
        self.players_df.loc[self.players_df['position'] == 'DEF', 'name'] = \
            self.players_df.loc[self.players_df['position'] == 'DEF', 'team'] + ' Defense'
                ...
                for proj in db_projections:
                    ...
                    if player:
                        projections[proj.player_id] = PlayerProjection(
                            ...
                            name=player.full_name or f"{player.first_name} {player.last_name}".strip(),
                            position=player.position,
                            team=player.team,
                            projected_points=float(proj.proj_fantasy_points_ppr or proj.proj_fantasy_points or 0),
        ...
        for _, player in self.players_df.iterrows():
            ...
            if self.use_database and self.data_service:
                ...
                player_stats = self.data_service.get_player_stats(
                    player['player_id'], 
                    self.sleeper_client.config.season
                )
            ...
            projected_points = self._calculate_projected_points(player, stats_dict, week)
    ...
    def _convert_stats_to_dict(self, player_stats) -> Dict:
        ...
        total_stats = {
            ...
            'fgm': 0, 'xpm': 0,
            'def_st_td': 0, 'def_st_int': 0, 'def_st_sack': 0
        }
    ...
    def _estimate_ownership_from_db(self, player) -> float:
        ...
        position_ownership = {
            'QB': 15.0, 'RB': 25.0, 'WR': 20.0,
            'TE': 12.0, 'K': 8.0, 'DEF': 10.0
        }
    ...
    def _calculate_injury_risk_from_db(self, player) -> float:
        ...
        risk_factors = {
            'Active': 0.1, 'Questionable': 0.4, 'Doubtful': 0.7,
            'Out': 1.0, 'IR': 1.0
        }
        return risk_factors.get(player.injury_status or 'Active', 0.1)

    def _save_projections_to_db(self, projections: Dict[str, PlayerProjection], week: int) -> None:
        """Save projections to database."""
        try:
            projections_data = []
            for player_id, proj in projections.items():
                projection_data = {
                    ...
                    'proj_fantasy_points': proj.projected_points * 0.9,  # Estimate non-PPR
    ...
    def _calculate_projected_points(self, player: pd.Series, stats: Dict, week: int = None) -> float:
    ...
    def _get_team_adjustments(self) -> Dict[str, float]:
        ...
        return {
            'BUF': 1.2, 'KC': 1.15, 'DAL': 1.1, 'SF': 1.1,
            'MIA': 1.05, 'CIN': 1.05, 'PHI': 1.05,
    ...
    def optimize_lineup(self, 
                       ...
                       num_lineups: int = 1) -> List[Dict[str, any]]:
    ...
    def _optimize_single_lineup(self, 
                               constraints: LineupConstraints,
                               objective: str,
                               exclude_players: set = None) -> Optional[Dict[str, any]]:
        ...
        for position in positions:
            position_players = [
                player_vars[pid] for pid, proj in self.projections.items()
        ...
        total_rb_wr_te = constraints.rb_count + constraints.wr_count + constraints.te_count + constraints.flex_count
    ...
    def analyze_current_lineup(self, roster_data: Dict = None) -> Dict[str, any]:
        ...
        analysis = {
            ...
            'current_total_points': sum(p['projected_points'] for p in current_projections),
        ...
        if optimal_lineup:
            analysis['improvement_potential'] = (
                optimal_lineup['total_projected_points'] - analysis['current_total_points']
            )
            ...
            for player_id in to_drop:
                if player_id in self.projections:
                    ...
                    analysis['recommendations'].append({
                        ...
                        'player': {'id': player_id, 'name': proj.name, 'position': proj.position},
    ...
    def export_lineups(self, lineups: List[Dict], output_file: str = "data/processed/optimized_lineups.json"):
...
def main():
    ...
    parser.add_argument("--objective", choices=['points', 'ceiling', 'floor'], default='points')
    ...
    if lineups:
        ...
        for i, lineup in enumerate(lineups, 1):
            ...
            for player in lineup['players']:
                print(f"  {player['position']}: {player['name']} ({player['projected_points']:.1f} pts)")
