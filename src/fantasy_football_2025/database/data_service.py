...
class DataService:
    ...
    def create_or_update_league(self, league_data: Dict[str, Any]) -> League:
        ...
        with session_scope() as session:
            league = (
                ...
                .filter_by(league_id=league_data["league_id"])
                .first()
            )
    ...
    def create_or_update_users(
        self, users_data: List[Dict[str, Any]], league_id: str
    ) -> List[User]:
        ...
        with session_scope() as session:
            for user_data in users_data:
                ...
                user = (
                    session.query(User).filter_by(user_id=user_data["user_id"]).first()
                )
    ...
    def get_players(
        self, position: str = None, team: str = None, active_only: bool = True
    ) -> List[Player]:
    ...
    # Roster operations
    def create_or_update_rosters(
        self, rosters_data: List[Dict[str, Any]], league_id: str
    ) -> List[Roster]:
        ...
        with session_scope() as session:
            for roster_data in rosters_data:
                roster = (
                    ...
                    .filter_by(roster_id=roster_data["roster_id"], league_id=league_id)
                    .first()
                )
                ...
                self._update_roster_players(
                    session, roster.roster_id, roster_data.get("players", [])
                )
    ...
    def _update_roster_players(
        self, session: Session, roster_id: int, player_ids: List[str]
    ) -> None:
        ...
        for player_id in player_ids:
            roster_player = RosterPlayer(
                roster_id=roster_id, player_id=player_id, added_date=datetime.utcnow()
            )
    ...
    def get_roster_players(self, roster_id: int) -> List[str]:
        ...
        with session_scope() as session:
            roster_players = (
                session.query(RosterPlayer).filter_by(roster_id=roster_id).all()
            )
    ...
    def get_user_roster(self, user_id: str, league_id: str) -> Optional[Roster]:
        ...
        with session_scope() as session:
            return (
                session.query(Roster)
                .filter_by(user_id=user_id, league_id=league_id)
                .first()
            )
    ...
    def get_player_stats(
        self, player_id: str, season: str, week: int = None
    ) -> List[PlayerStats]:
    ...
    def create_or_update_projections(
        self, projections_data: List[Dict[str, Any]]
    ) -> int:
    ...
    def get_projections(
        self, week: int, season: str = "2024", model_version: str = "default"
    ) -> List[PlayerProjection]:
        """Get projections for a specific week."""
        with session_scope() as session:
            return (
                session.query(PlayerProjection)
                .filter_by(week=week, season=season, model_version=model_version)
                .all()
            )

    # Transaction operations
    def create_transactions(
        self, transactions_data: List[Dict[str, Any]], league_id: str
    ) -> int:
        ...
        with session_scope() as session:
            for trans in transactions_data:
                ...
                existing = (
                    ...
                    .filter_by(transaction_id=trans.get("transaction_id"))
                    .first()
                )
    ...
    # Trending players
    def update_trending_players(
        self, trending_data: List[Dict[str, Any]], trend_type: str
    ) -> int:
        ...
        with session_scope() as session:
            ...
            session.query(TrendingPlayer).filter_by(
                trend_type=trend_type, date=today
            ).delete()
    ...
    def get_latest_analysis(
        self, league_id: str, run_type: str
    ) -> Optional[AnalysisRun]:
        """Get the latest analysis run of a specific type."""
        with session_scope() as session:
            return (
                session.query(AnalysisRun)
                .filter_by(league_id=league_id, run_type=run_type, status="completed")
    ...
    def get_available_players(
        self, league_id: str, position: str = None
    ) -> List[Player]:
        ...
        with session_scope() as session:
            ...
            query = session.query(Player).filter(
                ~Player.player_id.in_(rostered_players), Player.status == "Active"
            )
    ...
    def get_league_summary(self, league_id: str) -> Dict[str, Any]:
        ...
        with session_scope() as session:
            ...
            transaction_count = (
                session.query(Transaction).filter_by(league_id=league_id).count()
            )
