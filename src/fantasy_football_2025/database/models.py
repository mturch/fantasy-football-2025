...
from sqlalchemy.dialects.mysql import BIGINT, DECIMAL, MEDIUMTEXT
...
class League(Base):
    ...
    rosters = relationship(
        "Roster", back_populates="league", cascade="all, delete-orphan"
    )
    users = relationship("User", back_populates="league", cascade="all, delete-orphan")
    transactions = relationship(
        "Transaction", back_populates="league", cascade="all, delete-orphan"
    )
...
class User(Base):
    ...
    __table_args__ = (Index("idx_users_league", "league_id"),)
...
class Player(Base):
    ...
    projections = relationship(
        "PlayerProjection", back_populates="player", cascade="all, delete-orphan"
    )
...
class AnalysisRun(Base):
    ...
    )  # lineup_optimization, trade_analysis, waiver_analysis
    week = Column(Integer)
