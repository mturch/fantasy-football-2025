...
Examples:
  ...
  python -m src.main --league-id 123456 --user-id abc123 --full-analysis
    ...
    parser.add_argument("--output-dir", default="data/processed", help="Output directory for results")
    
    # Action arguments
    parser.add_argument("--pull-data", action="store_true", help="Pull all league data from Sleeper")
    parser.add_argument("--optimize-lineup", action="store_true", help="Generate optimal lineups")
    parser.add_argument("--analyze-trades", action="store_true", help="Find trade opportunities")
    parser.add_argument("--waiver-wire", action="store_true", help="Get waiver wire recommendations")
    ...
    try:
        ...
        print(f"   League type: {'PPR' if sleeper_client.is_ppr_league() else 'Standard'}")
    ...
    if not any([args.pull_data, args.optimize_lineup, args.analyze_trades, 
                args.waiver_wire, args.full_analysis]):
    ...
    try:
        ...
        if lineups:
            ...
            for i, lineup in enumerate(lineups, 1):
                ...
                print(f"   Floor: {lineup['total_floor']:.1f} | Ceiling: {lineup['total_ceiling']:.1f}")
                ...
                for pos in ['QB', 'RB', 'WR', 'TE', 'K', 'DEF']:
                    if pos in positions:
                        for player in positions[pos]:
                            print(f"   {pos}: {player['name']} ({player['projected_points']:.1f} pts)")
    ...
    try:
        ...
        if analysis.recommendations:
            ...
            for i, rec in enumerate(analysis.recommendations[:5], 1):
                ...
                print(f"   Matchup: {rec.matchup_rating:.2f} | Confidence: {rec.confidence_score:.2f}")
        ...
        if analysis.drop_candidates:
            ...
            for player in analysis.drop_candidates[:3]:
                print(f"   • {player['name']} ({player['position']}) - {player['drop_reason']}")
...
def show_quick_stats(sleeper_client: SleeperClient):
    ...
    try:
        ...
        recent_adds = [t for t in transactions if t.get('type') == 'waiver' and 
                      t.get('settings', {}).get('waiver_budget_used', 0) > 0]
