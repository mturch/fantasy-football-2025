...
import requests
import pandas as pd
from sleeper_api_wrapper import League, User, Players, Stats, Drafts
...
class SleeperClient:
    ...
    def get_rosters(self) -> List[Dict[str, Any]]:
        ...
        return self._get_cached_or_fetch(
            "rosters",
            lambda: self.league.get_rosters()
        )
    ...
    def get_matchups(self, week: int) -> List[Dict[str, Any]]:
        ...
        return self._get_cached_or_fetch(
            cache_key,
            lambda: self.league.get_matchups(week)
        )
    ...
    def get_players_data(self) -> Dict[str, Any]:
        ...
        return self._get_cached_or_fetch(
            "players_data",
            lambda: self.players.get_all_players()
        )

    def get_trending_players(self, sport: str = "nfl", add_type: str = "add") -> List[Dict[str, Any]]:
        """Get trending players (adds/drops)."""
        cache_key = f"trending_{add_type}"
        return self._get_cached_or_fetch(
            cache_key,
            lambda: self.players.get_trending_players(sport, add_type, hours=24, limit=50)
        )
    ...
    def get_transactions(self, week: int = None) -> List[Dict[str, Any]]:
        ...
        return self._get_cached_or_fetch(
            cache_key,
            lambda: self.league.get_transactions(week) if week else self.league.get_transactions()
        )
    ...
    def get_draft_picks(self, draft_id: str) -> List[Dict[str, Any]]:
        ...
        return self._get_cached_or_fetch(
            cache_key,
            lambda: self.drafts.get_all_picks(draft_id)
        )
    ...
    def save_to_database(self) -> None:
        ...
        try:
            ...
            self.data_service.create_or_update_rosters(rosters_data, self.config.league_id)
            logger.info(f"✅ Saved {len(rosters_data)} rosters")
            
            # Save transactions
            transactions_data = self.get_transactions()
            self.data_service.create_transactions(transactions_data, self.config.league_id)
    ...
    def get_my_roster_from_db(self) -> Optional[Dict[str, Any]]:
        ...
        roster = self.data_service.get_user_roster(self.config.user_id, self.config.league_id)
        if not roster:
            return None
...
def main():
    ...
    parser.add_argument("--output-dir", default="data/raw", help="Output directory for data")
