    ...
    def get_current_weather(self, lat: float, lon: float) -> Optional[WeatherData]:
        ...
        try:
            ...
            return WeatherData(
                ...
                dew_point=data["main"].get(
                    "feels_like"
                ),  # OpenWeatherMap doesn't provide dew point in free tier
                humidity=data["main"].get("humidity"),
                precipitation=(
                    data.get("rain", {}).get("1h", 0) if "rain" in data else 0
                ),
                wind_speed=data["wind"].get("speed"),
                wind_direction=data["wind"].get("deg"),
                pressure=data["main"].get("pressure"),
                condition=(
                    data["weather"][0]["description"] if data["weather"] else None
                ),
        ...
        try:
            ...
            for item in data["list"]:
                weather = WeatherData(
                    ...
                    condition=(
                        item["weather"][0]["description"] if item["weather"] else None
                    ),
    ...
    def get_current_weather(self, lat: float, lon: float) -> Optional[WeatherData]:
        ...
        try:
            ...
            return WeatherData(
                ...
                condition=(
                    current["condition"]["text"] if "condition" in current else None
                ),
                timestamp=datetime.fromisoformat(
                    current["last_updated"].replace("Z", "+00:00")
                ),
            )
            ...
            for day in data["forecast"]["forecastday"]:
                for hour in day["hour"]:
                    weather = WeatherData(
                        ...
                        condition=(
                            hour["condition"]["text"] if "condition" in hour else None
                        ),
                        timestamp=datetime.fromisoformat(
...
class WeatherDataPuller:
    ...
    def get_upcoming_games(self, days_ahead: int = 7) -> List[Dict]:
        ...
        try:
            with self.connection.get_connection() as conn:
                ...
                logger.info(
                    f"Found {len(games)} upcoming games in the next {days_ahead} days"
                )
                return games
    ...
    def save_weather_data(
        self, game_id: str, weather_data: WeatherData, source: str = "API"
    ) -> bool:
    ...
    def pull_weather_for_games(
        self, days_ahead: int = 7, include_forecast: bool = True
    ) -> Dict:
        ...
        try:
            ...
            for game in games:
                try:
                    logger.info(
                        f"Processing weather for game {game['game_id']} at {game['stadium_name']}"
                    )
                    ...
                    if include_forecast:
                        forecasts = self.weather_client.get_forecast(
                            game["latitude"], game["longitude"], days=min(days_ahead, 5)
                        )
                    ...
                    logger.error(
                        f"Failed to process weather for game {game['game_id']}: {e}"
                    )
                    results["errors"] += 1
            ...
            logger.info(
                f"Weather pull completed: {results['games_processed']} games, {results['weather_records']} records, {results['errors']} errors"
            )
            return results
        ...
        try:
            ...
            for stadium in stadiums:
                try:
                    ...
                    logger.error(
                        f"Failed to process weather for {stadium['stadium_name']}: {e}"
                    )
                    results["errors"] += 1
            ...
            logger.info(
                f"Stadium weather pull completed: {results['stadiums_processed']} stadiums, {results['weather_records']} records, {results['errors']} errors"
            )
            return results
...
def main():
    ...
    parser.add_argument("--api-key", required=True, help="Weather API key")
    parser.add_argument(
        "--api-provider",
    ...
    try:
        ...
        if args.stadiums_only:
            results = puller.pull_weather_for_stadiums(
                include_forecast=not args.no_forecast
            )
        else:
            results = puller.pull_weather_for_games(
                days_ahead=args.days_ahead, include_forecast=not args.no_forecast
            )
