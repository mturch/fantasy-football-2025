...
class HistoricalDataImporter:
    ...
    def connect_database(self) -> None:
        ...
        try:
            ...
            logger.info(f"Connected to database and ensured schema '{self.schema_name}' exists")
    ...
    def import_stadium_coordinates(self, file_path: str) -> int:
        ...
        try:
            ...
            df['stadium_azimuth_angle'] = pd.to_numeric(df['StadiumAzimuthAngle'], errors='coerce')
    ...
    def import_games_weather(self, file_path: str, batch_size: int = 1000) -> int:
        ...
        try:
            ...
            for chunk_num, df_chunk in enumerate(pd.read_csv(file_path, chunksize=batch_size)):
                logger.info(f"Processing chunk {chunk_num + 1} with {len(df_chunk)} records")
                
                # Clean and prepare data
                df_chunk['time_measure'] = pd.to_datetime(df_chunk['TimeMeasure'])
                df_chunk['temperature'] = pd.to_numeric(df_chunk['Temperature'], errors='coerce')
                df_chunk['dew_point'] = pd.to_numeric(df_chunk['DewPoint'], errors='coerce')
                df_chunk['humidity'] = pd.to_numeric(df_chunk['Humidity'], errors='coerce')
                df_chunk['precipitation'] = pd.to_numeric(df_chunk['Precipitation'], errors='coerce')
                df_chunk['wind_speed'] = pd.to_numeric(df_chunk['WindSpeed'], errors='coerce')
                df_chunk['wind_direction'] = pd.to_numeric(df_chunk['WindDirection'], errors='coerce')
                df_chunk['pressure'] = pd.to_numeric(df_chunk['Pressure'], errors='coerce')
                df_chunk['distance_to_station'] = pd.to_numeric(df_chunk['DistanceToStation'], errors='coerce')
    ...
    def validate_data(self) -> Dict[str, int]:
        ...
        try:
            with self.connection.get_connection() as conn:
                ...
                cursor.execute(f"SELECT COUNT(*) FROM {self.schema_name}.stadium_coordinates")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Import historical data to MySQL database")
    parser.add_argument(
        "--data-dir", 
    ...
    try:
        ...
        if args.validate_only:
            ...
            print(f"Date range: {validation['earliest_game']} to {validation['latest_game']}")
