...
def main():
    ...
    parser.add_argument(
        "--migrate-json", help="Migrate JSON files to database (provide directory path)"
    )
    parser.add_argument(
        "--export-json", help="Export database to JSON files (provide output directory)"
    )
    parser.add_argument(
        "--backup", help="Create database backup (provide backup directory)"
    )
    parser.add_argument(
        "--pull-fresh", action="store_true", help="Pull fresh data from Sleeper API"
    )
    parser.add_argument("--league-id", required=True, help="Sleeper league ID")
    parser.add_argument(
        "--user-id", help="Your Sleeper user ID (required for --pull-fresh)"
    )
